    ...
    match &cli.command {
        ...
        Commands::Loc { dir, src, .. } => {
                ...
                .filter(|x| x.is_ok() && x.as_ref().unwrap().file_type().unwrap().is_file())
            {
                ...
                let commit = relations.info.unwrap().commit;
                if let Some(relations) = relations.relations {
                    relations.into_iter().for_each(|x| {
                        let module = x.module;
                        use std::path::Path;
                        ...
                        let result = Command::new("scc")
                            .arg("-f json")
                            .arg("-c")
                            .current_dir(Path::new(src).join(module.clone()).join("src"))
                            .output()
                            .map_err(|x| {
                                eprintln!("issue with scc {}{}",Path::new(src).join(module.clone()).join("src").to_str().unwrap() , x);
                                x
                            })
                            .ok();

                        let result = result.and_then(|x| {
                            serde_json::from_slice::<Vec<I>>(&x.stdout)
                                .map_err(|e| {
                                    eprintln!(
                                        "issue with deserialization {} on {}",
                                        e,
                        ...
                        if let Some(result) =
                            result.and_then(|x| x.into_iter().find(|x| x.Name == "Java"))
                        {
                            res.push(R {
                                commit: commit.clone(),
                                module,
        ...
        Commands::MultiCompareStatsPerDecl {
            baseline_dir,
            evaluated_dir,
            json,
            ..
            ...
            let comps = files
                ...
                .filter_map(|(commit, (baseline, evaluated))| {
                    ...
                    Some(CommitCompStatsPerDecl::from(x))
                });
            if *json {
                let res: Vec<_> = comps.collect();
                println!("{}", serde_json::to_string_pretty(&res).unwrap());
...
#[derive(Serialize, Deserialize, Debug)]
struct CommitCompStatsPerDecl {
    relations_stats: Option<Vec<PerModule<ComparisonsPerDecl>>>,
    construction_perfs: Versus<Perfs>,
    search_perfs: Option<Versus<Perfs>>,
    info: Info,
}
...
#[derive(Serialize, Deserialize, Debug)]
pub struct CompResultsPerDecl {
    // decl: Position,
    #[serde(skip_serializing_if = "Option::is_none", default)]
    duration: Option<Versus<usize>>,
    #[serde(skip_serializing_if = "Option::is_none", default)]
    search: Option<Versus<String>>,
    exact_references: usize,
    remaining_refs_in_baseline: usize,
    remaining_refs_in_tool_results: usize,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct ResultsPerDecl {
    #[serde(skip_serializing_if = "Option::is_none", default)]
...
impl From<Relation> for ResultsPerDecl {
    fn from(x: Relation) -> Self {
        Self {
            duration: x.duration,
            search: x.search,
            references: x.refs.len(),
        }
    }
}

#[derive(Serialize, Deserialize, Debug)]
pub struct ComparisonsPerDecl {
    // pub(crate) api_version: String,
    pub(crate) left_name: String,
    pub(crate) right_name: String,
    /// declarations that match exactly
    #[serde(skip_serializing_if = "Vec::is_empty", default)]
    pub(crate) exact: Vec<CompResultsPerDecl>,
    #[serde(skip_serializing_if = "Vec::is_empty", default)]
    pub(crate) left: Vec<ResultsPerDecl>,
    #[serde(skip_serializing_if = "Vec::is_empty", default)]
    pub(crate) right: Vec<ResultsPerDecl>,
}

impl ComparisonsPerDecl {
    pub fn compute(comp: Comparisons) -> Self {
        ComparisonsPerDecl {
            ...
            exact: comp
                .exact
                .into_iter()
                .map(|x| CompResultsPerDecl {
                    // decl: x.decl,
                    duration: x.duration,
                    search: x.search,
                    exact_references: x.exact.len(),
                    remaining_refs_in_baseline: x.left.len(),
                    remaining_refs_in_tool_results: x.right.len(),
                })
                .collect(),
            left: comp.left.into_iter().map(|x| x.into()).collect(),
            right: comp.right.into_iter().map(|x| x.into()).collect(),
        }
    }
}

impl CommitCompStatsPerDecl {
    fn from(x: Versus<RelationsWithPerfs>) -> Self {
        let mut per_module: HashMap<String, (_, _)> = Default::default();
        ...
        let stats: Vec<_> = per_module
            ...
            .map(|(module, (bl_rs, t_rs))| {
                let comp = Comparator::default().compare(bl_rs, t_rs).into();
                let content = ComparisonsPerDecl::compute(comp);
                PerModule { module, content }
            })
            .collect();
...
enum Commands {
    ...
    /// LOC per Module per commit
    Loc { dir: String, src: String },
    ...
    /// Statistics on relations per decl
    MultiCompareStatsPerDecl {
        /// Directory that contains commit identified files that contains referential relations.
        /// It will be used as a baseline
        baseline_dir: String,
